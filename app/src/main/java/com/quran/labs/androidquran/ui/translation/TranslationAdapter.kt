    ...
    when {
      ...
      holder.text != null -> {
        ...
        if (row.type == TranslationViewRow.Type.SURA_HEADER) {
          ...
          val str = SpannableString(arabicText)
          str.setSpan(UthmaniSpan(context), 0, str.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)

          if (QuranSettings.getInstance(context).isTajweedColoringEnabled) {
            TajweedAnalyzer.analyze(arabicText).forEach { span ->
              str.setSpan(
                ForegroundColorSpan(Color.parseColor(span.rule.colorHex)),
                span.startIndex,
                span.endIndex,
